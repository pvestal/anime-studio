<template>
  <div>
    ...
    <div style="display: flex; gap: 0; margin-bottom: 16px; border-bottom: 1px solid var(--border-primary);">
      <button
        class="publish-subtab"
        :class="{ active: subtab === 'episodes' }"
        @click="subtab = 'episodes'"
      >
        Episodes
      </button>
      <button
    ...
    <PublishedLibrary v-if="subtab === 'library'" />
...
import PublishedLibrary from './publish/PublishedLibrary.vue'
...
const subtab = ref<'episodes' | 'library'>('episodes')
...
.publish-subtab:hover {
  color: var(--accent-primary);
}
.publish-subtab.active {
